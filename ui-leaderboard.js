...
(() => {
  ...
  function getLocalPlayerRow(ui) {
    const profile = ui?.storage && typeof ui.storage.getLeaderboardProfile === "function"
      ? ui.storage.getLeaderboardProfile()
      : null;
    if (!profile || profile.optIn !== true) return null;

    const pb = ui?.storage && typeof ui.storage.getPersonalBest === "function"
  ...
  function buildWindowRows(ui, windowType, remoteRows) {
    ...
    const sourceRows = Array.isArray(remoteRows) && remoteRows.length ? remoteRows : seedRows;
    const merged = LeaderboardLogic.mergeLocalPlayer(sourceRows, localPlayer, limit);
  ...
  async function refresh(ui, opts) {
    ...
    const baseUrl = String(cfg?.apiBaseUrl || "").trim().replace(/\/+$/, "");
    if (!baseUrl) {
      buildFallback(ui, "");
    ...
    const p = Promise.all([
      ...
      .then(([weeklyJson, allJson]) => {
        ...
        bucket.error = "";
        bucket.weekly = buildWindowRows(ui, "weekly", LeaderboardLogic.normalizeRows(weeklyJson?.top, clampInt(cfg?.topN, 1, 100)));
        bucket.all = buildWindowRows(ui, "all", LeaderboardLogic.normalizeRows(allJson?.top, clampInt(cfg?.topN, 1, 100)));
        bucket.lastFetchedAt = Date.now();
      })
      .catch((err) => {
  ...
  function ensureFresh(ui) {
    ...
    const ageMs = Date.now() - clampInt(bucket.lastFetchedAt, 0, Number.MAX_SAFE_INTEGER);
  ...
  function getLandingModel(ui) {
    ...
    return {
      loading: bucket?.loading === true && (!Array.isArray(bucket?.weekly) || bucket.weekly.length === 0),
      error: String(bucket?.error || "").trim(),
      source: String(bucket?.source || "").trim(),
      rows: Array.isArray(bucket?.weekly) ? bucket.weekly.slice(0, previewCount) : [],
      hasRows: Array.isArray(bucket?.weekly) && bucket.weekly.length > 0,
      hasProfile: profile?.optIn === true && !!String(profile?.nickname || "").trim(),
  ...
  function renderLandingCard(ui, helpers) {
    ...
    const sub = String(model.hasProfile ? (w.cardSubJoined || "") : (w.cardSubDefault || "")).trim();
    const ctaLabel = String(model.hasProfile ? (w.cardCtaView || "") : (w.cardCtaJoin || "")).trim();
  ...
  function setModalTab(ui, tabKey) {
    ...
    root.querySelectorAll("[data-wt-leaderboard-tab]").forEach((btn) => {
      const isActive = String(btn.getAttribute("data-wt-leaderboard-tab") || "") === nextTab;
    ...
    root.querySelectorAll("[data-wt-leaderboard-panel]").forEach((panel) => {
      const isActive = String(panel.getAttribute("data-wt-leaderboard-panel") || "") === nextTab;
  ...
  function openModal(ui, helpers) {
    const escapeHtml = helpers?.escapeHtml;
    if (typeof escapeHtml !== "function" || typeof ui?.openModal !== "function") return;
    ...
    const body = String(joined ? (w.modalBodyJoined || "") : (w.modalBodyDefault || "")).trim();
    ...
    const rankingTabLabel = String(w.rankingTab || weeklyTitle || title || "Leaderboard").trim();
    const profileTabLabel = String(w.profileTab || nicknameLabel || "Profile").trim();
  ...
  async function saveProfileFromModal(ui, helpers) {
    ...
    const w = getWording(ui);
    const cfg = getCfg(ui);
    const input = ui?.modalContentEl ? ui.modalContentEl.querySelector("#wt-leaderboard-nickname") : null;
    ...
    if (!nickname) {
      if (typeof toastNow === "function") {
        toastNow(ui.config, String(w.nicknameRequiredToast || "").trim(), { variant: "danger" });
      }
      return;
    }
    ...
    if (nickname.length < minLen) {
      if (typeof toastNow === "function") {
        toastNow(ui.config, String(w.nicknameTooShortToast || "").trim(), { variant: "danger" });
      }
      return;
    }
    ...
    if (nicknameRegex === false) {
      if (typeof toastNow === "function") {
        toastNow(ui.config, String(w.nicknameInvalidCharsToast || "").trim(), { variant: "danger" });
      }
      return;
    }
    ...
    if (baseUrl) {
      try {
        ...
        if (typeof toastNow === "function") {
          toastNow(ui.config, String(w.remoteSaveErrorToast || "").trim(), { variant: "info" });
        }
      }
    }
    ...
    if (typeof toastNow === "function") {
      toastNow(ui.config, String(w.saveOkToast || "").trim(), { variant: "success" });
    }
    try { ui.render(); } catch (_) { /* silent */ }
  }
  ...
  async function leaveFromModal(ui, helpers) {
    ...
    if (typeof toastNow === "function") {
      toastNow(ui.config, String(w.leftToast || "").trim(), { variant: "info" });
    }
    try { ui.render(); } catch (_) { /* silent */ }
  }

  async function submitRun(ui, lastRun, helpers) {
    if (!shouldSubmitScores(ui)) return { ok: false, skipped: true, reason: "disabled" };
    ...
    if (!profile || profile.optIn !== true) return { ok: false, skipped: true, reason: "not_opted_in" };
    ...
    if (!answers.length) return { ok: false, skipped: true, reason: "no_answers" };

    const getLeaderboardContentVersion = helpers?.getLeaderboardContentVersion;
    const contentVersion = (typeof getLeaderboardContentVersion === "function")
    ...
    try {
      ...
      return { ok: false, skipped: false, reason: String(err?.message || "submit_failed") };
    }
  }
