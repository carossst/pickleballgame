...
void (function () {
  ...
  UI.prototype._bindEvents = function () {
    ...
    function dispatchAction(action, event, actionEl) {
      try {
      ...
      switch (action) {
        ...
        case 'switch-leaderboard-tab':
          self.switchLeaderboardModalTab(
            actionEl && typeof actionEl.getAttribute === 'function'
              ? actionEl.getAttribute('data-wt-leaderboard-tab')
              : ''
          );
          break;
    ...
    if (this.modalEl && !this._wtBoundModalActions) {
      ...
      if (pointerEvt !== 'click') {
        const dedupHandler = (e) => {
          ...
          // Keep the anti-duplicate click guard, but do not make the first-run
          // modal feel unresponsive. The duplicate click generated by the same
          // pointer gesture arrives almost immediately; a real second tap/click
          // on the modal CTA can reasonably happen shortly after the modal opens.
